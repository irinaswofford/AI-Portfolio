...
TOKEN_FILE = "token.json"          # local token file
...
def load_credentials() -> Credentials:
    ...
    1. Uses TOKEN_JSON secret if available.
    2. Refreshes if expired.
    3. Falls back to local OAuth flow if necessary.
    """
    creds: Optional[Credentials] = None

    # Save credentials.json if provided as secret
    ...
    # Load from TOKEN_JSON secret
    token_json = os.getenv("TOKEN_JSON")
    if token_json:
        try:
            creds = Credentials.from_authorized_user_info(json.loads(token_json), SCOPES)
            # save locally for debugging
            _save_json(TOKEN_FILE, creds.to_json())
        except Exception:
            creds = None
    ...
    raise RuntimeError(
        ...
        "Provide CREDENTIALS_JSON/TOKEN_JSON secret or run local OAuth."
    )
